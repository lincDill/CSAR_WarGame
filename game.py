class Game:
    ...
    def trigger_player_effects(self, card):
        for effect in card.effects:
            if effect.get("trigger") == "on_effect_play":
                self.resolve_player_effects(effect, card)
    ...
    def resolve_effect(self, effect, source=None):
        ...
        if effect_type == "modify_strength":
            ...
            affected_targets = self.apply_strength_modifier(effect, source_threat=source_threat)
    ...
    def resolve_player_effects(self, effect, card):
        print(
    ...
    def apply_strength_modifier(self, effect, source_threat=None):
        ...
        if target == "support":
            ...
            if count is not None:
                selected_cards = self.choose_support_cards(count)
        ...
        if target == "threat":
            ...
            if count is not None:
                selected_threats = self.choose_threats_for_effect(count)
        ...
        if target in {"strike", "escort", "rescue", "c2"}:
            ...
            if count is not None:
                selected_cards = self.choose_support_cards(count, role=target)
    ...
    def choose_support_cards(self, count, role=None):
        attack_modes = {
            "attack_only",
            "dual_use",
            "attack_and_effect"
    ...
    def choose_threats_for_effect(self, count, category=None):
        eligible_threats = []
    ...
    def choose_setback_for_effect(self, count):
        if len(self.active_setbacks) == 0:
            print("\nNo active setbacks!")
            return []

        count = min(count, len(self.active_setbacks))
